...
% Compute individual cost boundary depending on angle
%angles_n = angles ./ max(angles);
angles_n = rescale(angles, 0, 1); 
ind_cost_boundary = vecnorm((og_bounds_right - og_bounds_left)')' * 0.5 .* angles_n;
ind_cost_boundary = max(bound_cost_min_meter, ind_cost_boundary);

% Get track boundaries
[bound_cost_start_left, bound_cost_start_right] = eps_dist_boundaries(og_bounds_left, og_bounds_right, ind_cost_boundary);
...
% Plot the racetrack with cost boundaries
if plot_loaded_dataset
    ...
    legend("Centerline", "Track Boundary", "Cost Boundary");
